        ...
        return (
          <div key={i} className="flex items-center">
            ...
            {i < STEP_LABELS.length - 1 && (
              <div className="w-5 sm:w-14 h-px mx-1 sm:mx-3 mb-5"
    ...
    <div>
      <div className="flex items-center justify-between mb-4">
        ...
        <div className="flex gap-2">
          <button
            className="p-1.5 text-[#C49A6C] disabled:opacity-30 hover:bg-[rgba(196,154,108,0.1)] rounded-md transition-colors"
          ...
          <button
            className="p-1.5 text-[#C49A6C] hover:bg-[rgba(196,154,108,0.1)] rounded-md transition-colors"
            onClick={() => setOffset(o => o + 1)}
          >
        ...
        {days.map(day => {
          ...
          return (
            <button
              ...
              className="aspect-square flex items-center justify-center text-sm font-medium disabled:opacity-30 disabled:cursor-not-allowed cursor-pointer rounded-md"
              style={{
...
export function Booking({ preselectServiceId }: BookingProps) {
  ...
  const { state, setService, goToStep2, selectKapper, goToStep3, setDate, setSlot, goToStep4, updateField, goBack, submit, reset } = useBooking()
  ...
  useEffect(() => {
    ...
    if (state.step !== prev) {
      const dir = state.step > prev ? 'step-enter-right' : 'step-enter-left'
      prevStepRef.current = state.step
      setStepAnimClass(dir)
      const t = setTimeout(() => setStepAnimClass(''), 350)
        ...
        >
          <div className={stepAnimClass}>
            ...
            {state.step === 1 && (
              <div className="flex flex-col">
                ...
                <p className="text-[#3D2B1F]/60 mb-8 text-sm" style={{ fontFamily: "'Lora', serif" }}>
                  Welke behandeling wil je boeken?
                </p>
            ...
            {state.step === 2 && (
              <div className="flex flex-col">
                <h3 className="text-2xl font-bold text-[#3D2B1F] mb-2"
                  style={{ fontFamily: "'Playfair Display', serif" }}>Kies je kapper</h3>
                <p className="text-[#3D2B1F]/60 mb-8 text-sm" style={{ fontFamily: "'Lora', serif" }}>
                  Wie mag jouw hoofd in handen nemen?
                </p>
                <div className="grid grid-cols-3 gap-2 sm:gap-4 flex-1">
                  {KAPPERS.map(kapper => {
                    const selected = state.kapper === kapper.id
                    return (
                      <button
                        key={kapper.id}
                        onClick={() => selectKapper(kapper.id)}
                        className="flex flex-col items-center gap-2 sm:gap-3 p-3 sm:p-6 rounded-xl focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#C49A6C]"
                        style={{
                          border: selected ? '2px solid #C49A6C' : '2px solid rgba(196,154,108,0.2)',
                          background: selected ? 'rgba(196,154,108,0.07)' : 'white',
                          transition: 'border-color 0.2s, background 0.2s',
                        }}
                        onMouseEnter={e => {
                          if (!selected) {
                            (e.currentTarget as HTMLButtonElement).style.borderColor = '#C49A6C'
                            ;(e.currentTarget as HTMLButtonElement).style.background = 'rgba(196,154,108,0.04)'
                          }
                        }}
                        onMouseLeave={e => {
                          if (!selected) {
                            (e.currentTarget as HTMLButtonElement).style.borderColor = 'rgba(196,154,108,0.2)'
                      ...
                      >
                        <div className="relative">
                          <img
                            ...
                            style={{
                              border: selected ? `3px solid ${kapper.colorHex}` : '3px solid rgba(196,154,108,0.2)',
            ...
            {state.step === 3 && (
              <div className="flex flex-col">
                ...
                <div className="grid md:grid-cols-2 gap-8 flex-1">
                  {/* Calendar */}
                  <div className="p-5 rounded-xl border border-[rgba(196,154,108,0.2)] bg-[#FDFAF5]">
                  ...
                  <div className="p-5 rounded-xl border border-[rgba(196,154,108,0.2)] bg-[#FDFAF5]">
                    ...
                    {!state.date ? (
                      ...
                      <div className="grid grid-cols-3 gap-2">
                        {[...Array(6)].map((_, i) => (
                          <div key={i} className="h-10 rounded-md bg-[rgba(196,154,108,0.08)] animate-pulse" />
                        ))}
                      </div>
                    ) : isBlocked ? (
                        ...
                        {slots.map((slot, i) => {
                          ...
                          return (
                            <button
                              ...
                              className="py-2.5 text-sm font-semibold uppercase tracking-wider rounded-md animate-slot"
                              style={{
                                fontFamily: "'Oswald', sans-serif",
            ...
            {state.step === 4 && (
              <div className="flex flex-col">
                ...
                <p className="text-[#3D2B1F]/60 mb-8 text-sm" style={{ fontFamily: "'Lora', serif" }}>
                  Vul je gegevens in en we zijn er.
                </p>

                {/* Summary */}
                <div className="bg-[#F5F0E8] p-4 mb-8 flex flex-wrap gap-6 text-sm rounded-xl border border-[rgba(196,154,108,0.2)]">
                  <div>
            ...
            {state.step === 5 && (
              <div className="text-center py-6 flex flex-col items-center">
                ...
                {/* Summary */}
                <div className="bg-[#F5F0E8] p-6 mb-8 text-left rounded-xl border border-[rgba(196,154,108,0.2)] max-w-sm w-full">
...
function ServiceCard({ service, selected, onSelect }: { service: Service; selected: boolean; onSelect: (s: Service) => void }) {
  return (
    <button
      ...
      className="text-left p-5 flex flex-col gap-3 rounded-xl cursor-pointer"
      style={{
    ...
    >
      <div className="flex items-start justify-between gap-2">
        ...
        {selected && (
          <div className="w-5 h-5 rounded-md flex items-center justify-center bg-[#C49A6C] flex-shrink-0 mt-0.5">
            <Check size={12} strokeWidth={2.5} className="text-[#1A1410]" />
          </div>
        )}
      </div>
      ...
      <div className="flex items-center gap-3">
        ...
        <span className="text-xs font-semibold text-[#A07848] uppercase tracking-widest border border-[rgba(196,154,108,0.3)] rounded-md px-2 py-0.5"
          style={{ fontFamily: "'Oswald', sans-serif" }}>
